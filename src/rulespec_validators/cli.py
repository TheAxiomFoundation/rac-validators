...
from rulespec_validators.consensus.engine import ConsensusEngine, ConsensusLevel
...
@click.group()
def cli():
    """Multi-system tax/benefit validation for RuleSpec DSL encodings."""
    pass
...
---
*This issue was automatically generated by rulespec-validators based on multi-system consensus analysis.*
"""
        console.print(
            Panel(
                f"[bold]{title}[/bold]\n\n{body[:500]}...",
...
def compare_aligned(year, output):
    ...
    """
    from rulespec_validators.comparison import run_aligned_comparison
    ...
    table.add_column("RuleSpec", justify="right")
    ...
    for var_result in dashboard["variables"]:
        ...
        table.add_row(
            ...
            f"${var_result['rulespec_weighted_total'] / 1e9:.1f}B",
...
def compare(year, tolerance, variables, output):
    """Compare RuleSpec vs PolicyEngine record-by-record on CPS data."""
    from rulespec_validators.comparison import run_full_comparison

    variables_list = list(variables) if variables else None

    console.print("\n[bold]RuleSpec vs PolicyEngine Record Comparison[/bold]")
...
def dashboard(year, output):
    ...
    axiom-foundation.org/validation dashboard page.

    Example:
        rulespec-validators dashboard -o validation-results.json
        cp validation-results.json ~/TheAxiomFoundation/axiom-foundation.org/public/data/
    """
    from pathlib import Path

    from rulespec_validators.dashboard_export import run_export
...
def harness_run(only, output, baseline):
    ...
    Example:
        rulespec-validators harness run
        rulespec-validators harness run --only quality
        rulespec-validators harness run -o current.json -b baselines/main.json
    """
    from rulespec_validators.harness.checkpoint import (
        load_checkpoint,
        save_checkpoint,
    )
    from rulespec_validators.harness.runner import run_harness
    from rulespec_validators.harness.scorecard import generate_compact_scorecard

    console.print("\n[bold]RuleSpec Validation Harness[/bold]")
    if only:
...
def harness_checkpoint(save, name):
    ...
    Example:
        rulespec-validators harness checkpoint --save baselines/main.json
        rulespec-validators harness checkpoint --name main
    """
    from rulespec_validators.harness.checkpoint import save_baseline
    from rulespec_validators.harness.runner import run_harness

    if save:
        console.print(f"[bold]Saving checkpoint to {save}[/bold]")
        result = run_harness()
        from rulespec_validators.harness.checkpoint import save_checkpoint
...
def harness_compare(baseline, current):
    ...
    Example:
        rulespec-validators harness compare -b baselines/main.json
    """
    from rulespec_validators.harness.checkpoint import (
        Checkpoint,
        compare_checkpoints,
...
def harness_scorecard(before, after, output):
    ...
    Example:
        rulespec-validators harness scorecard -b main.json -a current.json -o scorecard.md
    """
    from rulespec_validators.harness.checkpoint import load_checkpoint
    from rulespec_validators.harness.runner import run_harness
    from rulespec_validators.harness.scorecard import generate_scorecard
