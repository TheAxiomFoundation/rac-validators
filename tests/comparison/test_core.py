...
class TestCompareRecords:
    def test_perfect_match(self):
        rulespec = np.array([100.0, 200.0, 300.0])
        pe = np.array([100.0, 200.0, 300.0])
        result = compare_records(rulespec, pe, tolerance=1.0)
    ...
    def test_within_tolerance(self):
        rulespec = np.array([100.5, 200.5, 300.5])
        pe = np.array([100.0, 200.0, 300.0])
        result = compare_records(rulespec, pe, tolerance=1.0)
        assert result["match_rate"] == 1.0

    def test_mismatches(self):
        rulespec = np.array([100.0, 220.0, 300.0, 450.0])
        pe = np.array([100.0, 200.0, 300.0, 400.0])
        result = compare_records(rulespec, pe, tolerance=10.0)
        assert result["match_rate"] == 0.5
        assert result["n_mismatches"] == 2

    def test_error_percentiles(self):
        rulespec = np.array([100.0, 110.0, 120.0, 130.0, 200.0])
        pe = np.array([100.0, 100.0, 100.0, 100.0, 100.0])
        result = compare_records(rulespec, pe, tolerance=5.0)
    ...
    def test_mean_absolute_error(self):
        rulespec = np.array([100.0, 200.0, 300.0, 400.0])
        pe = np.array([100.0, 220.0, 300.0, 450.0])
        result = compare_records(rulespec, pe, tolerance=10.0)
        assert result["mean_absolute_error"] == pytest.approx(17.5)

    def test_worst_mismatches(self):
        rulespec = np.array([100.0, 200.0, 1000.0, 400.0])
        pe = np.array([100.0, 200.0, 300.0, 400.0])
        result = compare_records(rulespec, pe, tolerance=10.0, top_n_mismatches=3)
...
class TestLoadPeValues:
    ...
    def test_requires_policyengine(self):
        from rulespec_validators.comparison.core import HAS_POLICYENGINE
...
class TestLoadRuleSpecValues:
    def test_load_values(self, tmp_path):
        ...
        with (
            ...
            result = load_rulespec_values("income_tax", year=2024)
            assert len(result) == 3
    ...
    def test_load_values_with_ids(self, tmp_path):
        ...
        with (
            ...
            values, ids = load_rulespec_values("income_tax", year=2024, return_ids=True)
            assert len(values) == 3
            assert len(ids) == 3
    ...
    def test_load_variable_not_found(self, tmp_path):
        ...
        with (
            ...
            with pytest.raises(ValueError, match="not found"):
                load_rulespec_values("nonexistent_xyz", year=2024)
...
class TestRunFullComparison:
    def test_run_full_default(self):
        with patch("rulespec_validators.comparison.core.run_variable_comparison") as mock_rv:
    ...
    def test_run_full_handles_error(self):
        with patch("rulespec_validators.comparison.core.run_variable_comparison", side_effect=Exception("fail")):
            result = run_full_comparison(year=2024)
...
class TestAlignRecords:
    def test_align_matching_ids(self):
        rulespec_values = np.array([100.0, 200.0, 300.0])
        rulespec_ids = np.array([1, 2, 3])
        ...
        assert len(aligned_rulespec) == 3
        assert len(aligned_pe) == 3
        np.testing.assert_array_equal(aligned_rulespec, rulespec_values)
    ...
    def test_align_partial_overlap(self):
        ...
        aligned_rulespec, aligned_pe, matched_ids = align_records(rulespec_values, rulespec_ids, pe_values, pe_ids)
        assert len(aligned_rulespec) == 2
        assert len(aligned_pe) == 2
        np.testing.assert_array_equal(matched_ids, [2, 3])

    def test_align_no_overlap(self):
        rulespec_values = np.array([100.0])
        rulespec_ids = np.array([1])
        pe_values = np.array([200.0])
        pe_ids = np.array([2])
        with pytest.raises(ValueError, match="No matching"):
            align_records(rulespec_values, rulespec_ids, pe_values, pe_ids)
